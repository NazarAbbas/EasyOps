...
class EditWorkorderInfoController extends GetxController {
  // ─────────────────────────────────────────────────────────────────────────
  ...
  void applyAssetMetaFor(String input) {
    final hit = EditAssetsCatalog.find(input); // trims & uppercases inside
    if (hit != null) {
  ...
  List<String> get assetNumbers =>
      EditAssetsCatalog.items.map((e) => e.number).toList();
  ...
  Future<void> openAssetPicker(BuildContext context) async {
    await pickFromList(
      ...
      items: EditAssetsCatalog.items.map((e) => e.number).toList(),
...
class EditAsset {
  ...
  const EditAsset(this.number, this.type, this.description);
}

class EditAssetsCatalog {
  // In-memory catalog (sample data — replace with real values)
  static const List<EditAsset> items = [
    EditAsset(
      '1001',
      'High',
  ...
  static final Map<String, EditAsset> index = {
    for (final a in items) a.number.toUpperCase(): a,
  };

  static EditAsset? find(String number) => index[number.trim().toUpperCase()];
}
